              ...
              fluidPage(
                ...
                sidebarLayout(
                  sidebarPanel(
                    ...
                    sliderInput("top_n_words_network", "Top N Words (TF-IDF):", min = 20, max = 200, value = 100, step = 10),
                    sliderInput("cor_threshold", "Min Correlation Threshold:", min = 0.1, max = 0.9, value = 0.3, step = 0.05)
                  ...
                  mainPanel(
                    tabsetPanel(
                      ...
                      tabPanel("TF-IDF Network", plotOutput("tfidf_network_plot", height = "500px"))
                    )
                  )
                )
              )
...
server <- function(input, output, session) {
  ...
  cached_network_data <- reactiveVal(NULL)
  ...
  observeEvent(input$reload_lda, {
    ...
    word_tf_idf <- processed_data %>%
      ...
      distinct(LDA_Category, tender_no, word, .keep_all = TRUE) %>%  # << 加這行！
    ...
    output$tfidf_network_plot <- renderPlot({
      ...
      word_doc_pairs <- filtered_data() %>%
        ...
        distinct()  # 防止重複 document-word 組合
      
      # 計算共現關聯
      word_cor <- word_doc_pairs %>%
        pairwise_cor(item = word, feature = tender_no, sort = TRUE, upper = FALSE)
      ...
      ggraph(g, layout = "fr") +
        ...
        ## ✅ 存入快取以供下次直接顯示
        #cached_network_data(list(
        #  plot = p,
        #  n_words = input$top_n_words_network,
