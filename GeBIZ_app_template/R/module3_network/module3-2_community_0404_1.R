...
network_community_server <- function(id, dataset) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      cat("Executing line [259]\n")      
      cat("DEBUG: tender_cat exists in dataset:", "tender_cat" %in% names(dataset), "\n")
      if ("tender_cat" %in% names(dataset)) {
        cat("DEBUG: Number of unique tender_cat values:", length(unique(dataset$tender_cat)), "\n")
        cat("DEBUG: Unique tender_cat values:", paste(head(unique(dataset$tender_cat)), collapse = ", "), "\n")
      }
      #------------------------------------
      req(dataset)
...
#-----    
    ...
    output$community_metrics <- renderUI({
      ...
      fluidRow(
        shinydashboard::valueBox(
          n_communities, "Communities", icon = shiny::icon("network-wired"), 
          color = "light-blue", width = 3
        ),
        shinydashboard::valueBox(
          round(modularity_score, 3), "Modularity", icon = shiny::icon("project-diagram"), 
          color = "light-blue", width = 3
        ),
        shinydashboard::valueBox(
          round(within_density, 3), "Within Density", icon = shiny::icon("connectdevelop"), 
          color = "light-blue", width = 3
        ),
        shinydashboard::valueBox(
          round(between_density, 3), "Between Density", icon = shiny::icon("share-alt"), 
          color = "light-blue", width = 3
        )
      )
    })
    ...
    output$community_detail_metrics <- renderUI({
      ...
      fluidRow(
        valueBox(
          n_nodes, "Community Members", icon = shiny::icon("users", lib = "font-awesome"), 
          color = "light-blue", width = 3
        ),
        valueBox(
          n_agencies, "Agencies", icon = shiny::icon("building", lib = "font-awesome"), 
          color = "light-blue", width = 3
        ),
        valueBox(
          n_suppliers, "Suppliers", icon = shiny::icon("truck", lib = "font-awesome"), 
          color = "light-blue", width = 3
        ),
        valueBox(
          round(density, 3), "Internal Density", icon = shiny::icon("project-diagram", lib = "font-awesome"), 
          color = "light-blue", width = 3
        )
      )
    })
...
server <- function(input, output, session) {
  ...
  # Specifically rename tender_cat to tender_type
  if ("tender_cat" %in% names(community_data_processed)) {
    community_data_processed <- community_data_processed %>%
      rename(tender_type = tender_cat)
    cat("Using tender_cat as tender_type\n")
  } else {
    # If tender_cat doesn't exist, provide a warning
    cat("WARNING: tender_cat column not found in data\n")
    # Create a default tender_type column if needed
    community_data_processed$tender_type <- "Unknown"
  }
  
  # Add debugging to verify tender_type exists and has values
  cat("tender_type column exists:", "tender_type" %in% names(community_data_processed), "\n")
  if ("tender_type" %in% names(community_data_processed)) {
    cat("Unique tender_type values:", paste(head(unique(community_data_processed$tender_type)), collapse=", "), "\n")
  }
