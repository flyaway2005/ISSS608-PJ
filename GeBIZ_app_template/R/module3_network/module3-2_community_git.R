    ...
    fluidRow(
      ...
      column(width = 3,
             div(class = "control-panel",
                 ...
                 sliderInput(ns("award_amount"), "Award Amount (S$):", min = 0, max = 1500000000, 
                             value = c(0, 1500000000), step = 10000,
                             pre = "$", sep = ",", animate = FALSE,
                             width = "100%"),
                 # Container div for the two numeric inputs aligned with slider ends
                 div(
                 ...
                 selectInput(ns("centrality_measure"), "Centrality for Node Size:",
                             ...
                             selected = "Degree"),
#                 checkboxInput(ns("filter_small_communities"), "Filter small communities", FALSE),
#                 conditionalPanel(
#                   condition = paste0("input['", ns("filter_small_communities"), "'] == true"),
#                   sliderInput(ns("min_community_size"), "Minimum community size:", 
             ...
             tabsetPanel(
               tabPanel("Community Network Overview",
                        ...
                        fluidRow(
                          column(12, 
                                 ...
                                 visNetworkOutput(ns("community_network"), height = "500px"))
                        ),
               ...
               tabPanel("Community Explorer",
                        ...
                        fluidRow(
                          column(12, 
                                 h4("Community Network"),
                                 visNetworkOutput(ns("ego_community_network"), height = "500px"))
                        ),
                        fluidRow(
                          column(12, 
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      tryCatch({
        ...
        req_cols <- c("agency", "supplier_name", "agency_type", "tender_type")
        ...
        cat("DEBUG: Column names in dataset:", paste(names(data), collapse = ", "), "\n")
        
        # Print sample values to verify data
        cat("DEBUG: Sample agencies:", paste(head(unique(data$agency)), collapse = ", "), "\n")
        cat("DEBUG: Sample suppliers:", paste(head(unique(data$supplier_name)), collapse = ", "), "\n")
...
#-----    
    ...
    output$community_network <- renderVisNetwork({
      ...
      # # Filter small communities if specified
      # if(input$filter_small_communities) {
      #   community_sizes <- graph %>%
      #     activate(nodes) %>%
      #     as_tibble() %>%
      #     count(community) %>%
      #     filter(n >= input$min_community_size)
      #   
      #   # Get node ids that belong to the filtered communities
      #   valid_nodes <- graph %>%
      #     activate(nodes) %>%
      #     as_tibble() %>%
      ...
      #     pull(name)
      #   
      #   # Filter both nodes and corresponding edges
      #   graph <- graph %>%
      #     activate(nodes) %>%
      #     filter(name %in% valid_nodes) %>%
      #     # This is crucial: filter edges that connect filtered nodes
      #     activate(edges) %>%
      #     filter(from %in% valid_nodes & to %in% valid_nodes)
      #   
      #   cat("Filtered small communities, remaining nodes:", igraph::gorder(graph), 
      #       "remaining edges:", igraph::gsize(graph), "\n")
      #   
      #   # Check if we still have nodes after filtering
      #   if(igraph::gorder(graph) == 0 || igraph::gsize(graph) == 0) {
      #     return(visNetwork(nodes = data.frame(id = 1, label = "No data"),
      #                       edges = data.frame(from = 1, to = 1)) %>%
      #              addFontAwesome() %>%
      #              visNodes(physics = FALSE) %>%
      ...
      #              visEdges(width = 0) %>%
      #              visLegend(enabled = FALSE) %>%
      #              addNodesIterator(label = "No communities meet the minimum size criteria.\nTry reducing the minimum community size."))
      #   }
      # }
