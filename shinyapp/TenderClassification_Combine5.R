...
server <- function(input, output, session) {
  ...
  lda_results <- reactiveVal(NULL)  # 新增 LDA 結果的 reactive value
  ...
  observeEvent(input$run_market_analysis, {
    ...
    if (nrow(market_data) == 0) {
      showNotification("No data available for the selected date range", type = "warning")
      return(NULL)
    }
    ...
    # 📦 顯示 Market Overview 的三個 valueBox
