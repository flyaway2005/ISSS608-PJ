...
# Step 2: 统一列名，去掉可能的空格或特殊字符
colnames(GP) <- str_trim(colnames(GP))
colnames(EN) <- str_trim(colnames(EN))

# Step 3: 标准化 `agency` 和 `NAME`（转换为小写、去掉特殊字符和多余空格）
standardize_name <- function(name) {
  name %>%
    str_to_lower() %>%    # 轉小寫
    str_trim() %>%        # 去除首尾空格
    str_replace_all("[^a-z0-9 ]", " ") %>%  # 移除特殊字符並轉換為空格
    str_squish()          # 合併多餘空格
}

GP <- GP %>% mutate(agency_std = standardize_name(agency))
EN <- EN %>% mutate(NAME_std = standardize_name(NAME))

# Step 4: 确保 `SUBCAT` 为空时使用 `CATEGORY`
EN <- EN %>%
  mutate(subcat_filled = ifelse(is.na(SUBCAT) | SUBCAT == "", CATEGORY, SUBCAT))

# Step 5: 进行匹配
merged_data <- GP %>%
  left_join(EN, by = c("agency_std" = "NAME_std")) %>%
  select(all_of(intersect(colnames(GP), c("tender_no", "tender_description", "agency", "award_date", "tender_detail_status", "awarded_amt"))), subcat_filled)

# Step 6: 找出未匹配的机构
unmatched_agencies <- GP %>%
  filter(!agency_std %in% EN$NAME_std) %>%
  select(agency) %>%
  distinct()
...
print("未匹配的机构:")
print(unmatched_agencies)

# 保存结果
write_csv(merged_data, "data/processed_GovernmentProcurement.csv")
write_csv(unmatched_agencies, "data/unmatched_agencies.csv")

```

```{r}
matched_agencies <- intersect(GP$agency_std, EN$NAME_std)
print("成功匹配的机构:")
print(matched_agencies)

print("未匹配的机构:")
print(setdiff(GP$agency_std, EN$NAME_std))
...
unmatched_GP <- GP %>%
  filter(!agency_std %in% EN$NAME_std) %>%
  select(agency, agency_std) %>%
  distinct()

# 嘗試模糊匹配
matches <- stringdist_join(unmatched_GP, EN, by = c("agency_std" = "NAME_std"), method = "jw", max_dist = 0.2)
...
```{r}
matches %>% count(agency_clean) %>% filter(n > 1)
...
```{r}
write_csv(Cleaned_GP, "Cleaned_GP.csv")
```
